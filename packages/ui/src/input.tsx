...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  const state: InputState =
    stateProp ?? (ctx?.state as InputState | undefined) ?? "default";
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={cn(
            "h-8 w-full rounded-lg border bg-white text-sm text-zinc-900 outline-none transition-all duration-150 placeholder:text-zinc-400 disabled:cursor-not-allowed disabled:bg-zinc-50 disabled:text-zinc-400",
            leftIcon != null ? "pl-8" : "pl-3",
            trailingIcon != null ? "pr-8" : "pr-3",
            borderStyles[state],
            className,
          )}
          {...props}
      ...
      {hint && <p className={cn("text-xs", hintStyles[state])}>{hint}</p>}
    </div>
  );
});
