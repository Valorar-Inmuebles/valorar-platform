...
import { updateDevelopmentImageAction } from "@/lib/api/development-image-actions";
import type {
...
} from "@/lib/development/image-form";

type DevelopmentImageFormProps = {
...
export function DevelopmentImageForm({
  developmentId,
  image,
  onSuccess,
  onCancel,
}: DevelopmentImageFormProps) {
  const { toast } = useToast();
  const [values, setValues] = useState<DevelopmentImageEditFormValues>(() =>
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await updateDevelopmentImageAction(
        developmentId,
        image.id,
