...
import {
  deleteDevelopmentImageAction,
  markDevelopmentImageCoverAction,
  reorderDevelopmentImagesAction,
} from "@/lib/api/development-image-actions";
import type { AdminDevelopmentImage } from "@/lib/api/types/development-image";
import {
  buildReorderItems,
  sortImagesByOrder,
} from "@/lib/development/image-upload";

type DevelopmentImageGridProps = {
  developmentId: string;
  images: AdminDevelopmentImage[];
  onEdit: (image: AdminDevelopmentImage) => void;
...
function ImagePreview({ image }: { image: AdminDevelopmentImage }) {
  if (image.url) {
    return (
...
export function DevelopmentImageGrid({
  developmentId,
  images,
  onEdit,
  pendingActionId,
  ...
  const handleMarkCover = (image: AdminDevelopmentImage) => {
    if (image.isCover) return;

    setPendingActionId(image.id);
    startTransition(async () => {
      const result = await markDevelopmentImageCoverAction(
        developmentId,
        image.id,
      );
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteDevelopmentImageAction(
        developmentId,
  ...
  const persistReorder = (nextImages: AdminDevelopmentImage[]) => {
    setPendingActionId("reorder");
    startTransition(async () => {
      const result = await reorderDevelopmentImagesAction(
        developmentId,
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setDeleteTarget({
                        image,
                        willPromoteOther:
                          image.isCover && orderedImages.length > 1,
                      })
                    }
                  >
                    Eliminar
