...
function groupFeaturesByCategory(catalog: AdminPropertyFeature[]) {
  return DEVELOPMENT_FEATURE_CATEGORIES.map((category) => ({
    category,
...
export function DevelopmentFeatureManager({
  developmentId,
  catalog,
  assignments,
}: DevelopmentFeatureManagerProps) {
  ...
  const buildingCatalog = useMemo(
    () => catalog.filter((f) => DEVELOPMENT_FEATURE_CATEGORIES.includes(f.category)),
    [catalog],
  );
  ...
  const buildingAssignments = useMemo(
    () =>
      assignments.filter((a) =>
        DEVELOPMENT_FEATURE_CATEGORIES.includes(a.category),
      ),
    [assignments],
  );

  const [selectedIds, setSelectedIds] = useState<Set<string>>(
    () => new Set(buildingAssignments.map((assignment) => assignment.featureId)),
  );

  const assignmentSignature = buildingAssignments
  ...
  useEffect(() => {
    ...
    setError(null);
  }, [assignmentSignature, buildingAssignments]);

  const groups = useMemo(
    () => groupFeaturesByCategory(buildingCatalog),
    [buildingCatalog],
  );
  const selectedCount = selectedIds.size;
  const isDirty = useMemo(() => {
    const initial = new Set(buildingAssignments.map((a) => a.featureId));
    if (initial.size !== selectedIds.size) return true;
    for (const id of selectedIds) {
      if (!initial.has(id)) return true;
    }
    return false;
  }, [buildingAssignments, selectedIds]);
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const result = await replaceDevelopmentFeatureAssignmentsAction(
        developmentId,
        {
    ...
    return (
      <Card>
        <CardContent className="py-10 text-center text-sm text-muted">
          No hay características de edificio disponibles en el catálogo.
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted">
          Amenities y servicios del edificio. Las características de ambientes se
          configuran en cada unidad.
        </p>
        <Button
          onClick={handleSave}
          loading={isPending}
